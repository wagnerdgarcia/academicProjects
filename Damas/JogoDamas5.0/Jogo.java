...
public class Jogo {
    ...
    public void moverPeca(int origemX, int origemY, int destinoX, int destinoY) {
        ...
        else if ((tipoDePeca == 1) && (vezJogar == true)){
            /**
             * Movimento de Jogo das peças Brancas
             */
            if (((destinoX%2 == 0) && destinoY % 2 == 0) ||((destinoX%2 != 0) && destinoY % 2 != 0)){ 
    ...
    public void verificaCaptura(Casa casa){
        int casaX = casa.posicaoX();
        int casaY = casa.posicaoY();
        Peca PecaEmJogo = casa.getPeca();
        int tipoPeca = PecaEmJogo.getTipo();
        Casa objetivo1 = new Casa(casaX+1,casaY+1);
        boolean alvo1 = objetivo1.possuiPeca();
        if (alvo1){
            Casa destino = new Casa(casaX+2,casaY+2);
            boolean parada = destino.possuiPeca();
            if (!parada){
                PecaEmJogo.mover(destino);
                objetivo1.removerPeca();
                verificaCaptura(destino);
            }
        }
        Casa objetivo2 = new Casa(casaX-1,casaY+1);
        boolean alvo2 = objetivo2.possuiPeca();
        if (alvo2){
            Casa destino = new Casa(casaX-2,casaY+2);
            boolean parada = destino.possuiPeca();
            if (!parada){
                PecaEmJogo.mover(destino);
                objetivo2.removerPeca();
                verificaCaptura(destino);
            }
        }
        Casa objetivo3 = new Casa(casaX+1,casaY-1);
        boolean alvo3 = objetivo3.possuiPeca();
        if (alvo3){
            Casa destino = new Casa(casaX+2,casaY-2);
            boolean parada = destino.possuiPeca();
            if (!parada){
                PecaEmJogo.mover(destino);
                objetivo3.removerPeca();
                verificaCaptura(destino);
            }
        }
        Casa objetivo4 = new Casa(casaX-1,casaY-1);
        boolean alvo4 = objetivo4.possuiPeca();
        if (alvo4){
            Casa destino = new Casa(casaX-2,casaY-2);
            boolean parada = destino.possuiPeca();
            if (!parada){
                PecaEmJogo.mover(destino);
                objetivo4.removerPeca();
                verificaCaptura(destino);
            }
        }
    }
